        ...
        <v-tabs :vertical="$vuetify.breakpoint.smAndUp">
          ...
          <v-tab-item>
            <v-card class="pl-5 pr-5 pb-5" flat>
                ...
                <template>
                  ...
                  <v-data-table disable-sort v-if="preferences && preferences[0] && preferences[0].preference_items"
                    ...
                    <template v-slot:[`item.l1`]="{ item }">
                      <v-checkbox v-model="item.l1"
                        ...
                        @click="noviceToggle(preferences[0].preference_items.indexOf(item), item.l1)" color="success">
                      </v-checkbox>
                    </template>
                    <template v-slot:[`item.l2`]="{ item }">
                      <v-checkbox v-model="item.l2"
                        v-if="item.category !== 'Nutrition' && item.category !== 'Ergonomics' && item.category !== 'Recovery'"
                        :disabled="item.l1 || item.l3 || item.l4 || item.l5 || item.not_interested"
                        @click="beginnerToggle(preferences[0].preference_items.indexOf(item), item.l2)" color="success">
                      </v-checkbox>
                    </template>
                    <template v-slot:[`item.l3`]="{ item }">
                      <v-checkbox v-model="item.l3"
                        v-if="item.category !== 'Nutrition' && item.category !== 'Ergonomics' && item.category !== 'Recovery'"
                    ...
                    <template v-slot:[`item.l5`]="{ item }">
                      <v-checkbox v-model="item.l5"
                        v-if="item.category !== 'Nutrition' && item.category !== 'Ergonomics' && item.category !== 'Recovery'"
                        :disabled="item.l1 || item.l2 || item.l3 || item.l4 || item.not_interested"
                        @click="expertToggle(preferences[0].preference_items.indexOf(item), item.l5)" color="success">
                      </v-checkbox>
                    </template>
                    <template v-slot:[`item.not_interested`]="{ item }">
                      <v-checkbox v-model="item.not_interested"
                  ...
                  <v-data-table disable-sort v-else :headers="headers" :items="tablePreferences.preference_items"
                    ...
                    <template v-slot:[`item.l1`]="{ item }">
                      <v-checkbox
                        v-if="item.category !== 'Nutrition' && item.category !== 'Ergonomics' && item.category !== 'Recovery'"
                        v-model="item.l1" :disabled="item.l2 || item.l3 || item.l4 || item.l5 || item.not_interested"
                        @click="noviceToggle(tablePreferences.preference_items.indexOf(item), item.l1)" color="success">
                      </v-checkbox>
                    </template>
                    <template v-slot:[`item.l2`]="{ item }">
                      <v-checkbox
                        v-if="item.category !== 'Nutrition' && item.category !== 'Ergonomics' && item.category !== 'Recovery'"
                        v-model="item.l2" :disabled="item.l1 || item.l3 || item.l4 || item.l5 || item.not_interested"
                    ...
                    <template v-slot:[`item.l3`]="{ item }">
                      <v-checkbox
                        v-if="item.category !== 'Nutrition' && item.category !== 'Ergonomics' && item.category !== 'Recovery'"
                        v-model="item.l3" :disabled="item.l1 || item.l2 || item.l4 || item.l5 || item.not_interested"
                    ...
                    <template v-slot:[`item.l5`]="{ item }">
                      <v-checkbox
                        v-if="item.category !== 'Nutrition' && item.category !== 'Ergonomics' && item.category !== 'Recovery'"
                        v-model="item.l5" :disabled="item.l1 || item.l2 || item.l3 || item.l4 || item.not_interested"
                        @click="expertToggle(tablePreferences.preference_items.indexOf(item), item.l5)" color="success">
                      </v-checkbox>
                    </template>
                    <template v-slot:[`item.not_interested`]="{ item }">
                      <v-checkbox v-model="item.not_interested"
              ...
              <v-card-actions class="d-flex justify-end">
                <v-btn @click="savePreferences" class="px-10" color="primary" :block="$vuetify.breakpoint.xsOnly">Save
                </v-btn>
              </v-card-actions>
            </v-card>
          </v-tab-item>
