  ...
  <v-card elevation="8">
    ...
    <v-dialog v-model="play"  width="auto" overlay-opacity="0.9">
      <v-card width="1200">
        ...
        <v-card-title class="d-flex justify-space-between align-start flex-nowrap">
          <div>
            {{ video.snippet.title }}
          </div>
          <div>
            <v-btn v-if="favoriteReference" @click="removeFavorite" color="var(--mh-orange)"
              title="Remove from favorites" icon>
